...
> Application-selected requests intentionally include `adp_queryEngine_engineName: ""` alongside `adp_queryEngine_applicationIdentifier` for live ADP compatibility.
...
> engineName and applicationIdentifier are mutually exclusive selectors. Exactly one must be provided.
>
> The same task-specific exception to the normal sparse-request rule applies to CLI-generated requests: when `--applicationIdentifier` is used, generated requests intentionally include `adp_queryEngine_engineName: ""` alongside `adp_queryEngine_applicationIdentifier` for live ADP compatibility.
