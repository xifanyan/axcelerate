...
> engineName and applicationIdentifier are mutually exclusive selectors. Exactly one must be provided.
>
> When `--applicationIdentifier` is used, generated requests still treat it as the single effective selector and intentionally include `adp_queryEngine_engineName: ""` to clear the live ADP server default.
